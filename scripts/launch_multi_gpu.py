...
# -*- coding: utf-8 -*-
"""
One-click launch of multiple OpenPI inference service instances,
each bound to a different GPU and port.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="One-click launch of multiple OpenPI inference service instances (multi-GPU / multi-port)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--config", type=str, default="pi05_u0bot",
        help="OpenPI training config name (default: pi05_u0bot)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Default prompt if not provided in observation",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--script-path", type=str, default="scripts/inference_service_openpi.py",
        help="Inference service script path (default: scripts/inference_service_openpi.py)",
    )
...
def main():
    ...
    for i in range(num_instances):
        ...
        # Create a separate log file for each instance
        log_path = os.path.join(log_dir, f"openpi_inference_{timestamp}_inst{i}_gpu{gpu_id}_port{port}.log")
    ...
    print(f"\n{'='*60}")
    print(f"Launched {num_instances} OpenPI inference service instances:")
    print(f"{'='*60}")
